...
- 已提交 Git 基线：`8bcc0ed J-D1a`
- 当前工作状态：Step J-D1b 已完成并独立复核；工作区包含 J-D1b 源码与测试的未提交改动
...
当前已完成统一阶段、公开牌面事实、硬归属域、受控残局分配、离线评测、公开事件、多种子采集器、forced/战略性 pass 正式对照，并已用独立 corpus 证明 neutral baseline 不受 pass 策略分布影响。J-D1a/J-D1b 已为完整残局分配建立精确物理权重以及 token/rank 整数边际，下一步进入 J-D1c1 单样本离线概率评分。
...
| 残局精确分配 | Step J-D1b 完成 | 不超过 12 张时枚举完整分配，聚合上下界、物理权重与 token/rank 整数边际 | 尚无概率校准、置信度或主链接入 |
...
J-D1c1 先在 `evaluation/` 建立单样本、有理数可审计的评分充分统计量；J-D1c2 再使用独立多种子语料做分桶校准。
...
状态：J-A 至 J-D1b 已完成并核验，下一步实施 J-D1c1。
...
- Step J-D1c1：建立 evaluation-only 单样本概率评分与精确分桶充分统计量，下一步；
- Step J-D1c2：接入独立多种子语料，聚合 Brier、可靠性分桶与外部牌数分层；
- Step J-D1c3：预注册校准门槛并决定是否允许生成 runtime 置信度；
