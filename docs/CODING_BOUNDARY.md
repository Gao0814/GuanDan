...
- 允许在 `agents/` 内基于公开 observation 实现统一阶段分类、牌面信念状态和策略路由
- 允许为这些能力新增对应单元测试和离线评测辅助
- 允许做确定性约束传播和有限可行分配枚举
- 不允许读取其他玩家真实手牌作为运行时输入
- 不允许把牌面推断写入规则引擎或作为合法性真值
...
9. 阶段分类、牌面信念和策略路由属于 AI 层，不属于 `engine/`
10. 推断结论必须区分公开事实、硬约束和软信号
11. RAG 为策略提供知识证据，不作为唯一策略路由器
