...
状态：既有 invalid/inconclusive 永久保留。L4-A2c5a 已完成可审计零网络 preflight 契约，判定 verified；下一步先独立封存三个文件，再执行 L4-A2c5b 唯一真实环境 instrumented preflight。详细设计见 `docs/BOTZONE_INTEGRATION_PLAN.md`。
...
7. Phase 4 恢复：launcher 与 instrumented preflight 契约均已离线通过；下一步 L4-A2c5b 单次真实环境零网络 preflight，之后仍需 L4-A2c5c launcher offline 准入；
